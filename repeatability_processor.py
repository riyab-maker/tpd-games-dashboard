#!/usr/bin/env python3
"""
Repeatability Data Processor

This script processes raw data to generate repeatability metrics:
- Game repeatability analysis using hybrid database tables
- User counts by number of distinct games played
...
def fetch_repeatability_data() -> pd.DataFrame:
    ...
    try:
        ...
        print(f"✅ Fetched {len(hybrid_df)} records from hybrid database")
        return hybrid_df
        
    except Exception as e:
        print(f"❌ Error fetching hybrid data: {e}")
...
def fetch_matomo_repeatability_data() -> pd.DataFrame:
    ...
    try:
        ...
        print(f"✅ Fetched {len(df)} records from Matomo")
        return df
        
    except Exception as e:
        print(f"❌ Error fetching Matomo data: {e}")
        sys.exit(1)

def process_hybrid_repeatability_data(df: pd.DataFrame) -> pd.DataFrame:
    """Process hybrid database repeatability data"""
    print("📊 Processing hybrid repeatability data...")
    
    # Group by number of distinct games played
    ...
    print(f"✅ Hybrid repeatability: {len(repeatability_data)} records")
    return repeatability_data

def process_matomo_repeatability_data(df: pd.DataFrame) -> pd.DataFrame:
    """Process Matomo repeatability data (fallback)"""
    print("📊 Processing Matomo repeatability data...")
    ...
    print(f"✅ Matomo repeatability: {len(repeatability_data)} records")
    return repeatability_data
...
def save_processed_data(repeatability_data: pd.DataFrame):
    ...
    print(f"✅ Saved repeatability data to {file_path}")
...
def main():
    ...
    try:
        ...
        if 'CountDistinctNonNull_game_name' in df.columns:
            # Process hybrid data
            repeatability_data = process_hybrid_repeatability_data(df)
        else:
            # Process Matomo data
            repeatability_data = process_matomo_repeatability_data(df)
        ...
        print(f"❌ Error in repeatability processing: {e}")
        sys.exit(1)
