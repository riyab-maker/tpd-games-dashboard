#!/usr/bin/env python3
"""
Generate only time series data for immediate dashboard testing
"""
import os
import pandas as pd
import pymysql
...
def fetch_visits_users_data():
    ...
    try:
        ...
        print(f"SUCCESS: Fetched {len(df)} visits/users records")
        return df
    finally:
        connection.close()
...
def fetch_instances_data():
    ...
    try:
        ...
        print(f"SUCCESS: Fetched {len(df)} instances records")
        return df
    finally:
        connection.close()
...
def process_visits_users_data(df):
    ...
    # Convert idvisitor to proper readable format (binary to decimal)
    def convert_idvisitor(x):
        try:
            ...
            # Handle string representation
                ...
                clean_hex = ''.join(c for c in x if c in '0123456789abcdefABCDEF')
                if clean_hex:
    ...
    df['idvisitor_converted'] = df['idvisitor'].apply(convert_idvisitor)
    ...
    print(f"SUCCESS: Processed {len(df)} visits/users records")
    return df
...
def process_instances_data(df):
    ...
    print(f"SUCCESS: Processed {len(df)} instances records")
    return df
...
def aggregate_data(visits_users_df, instances_df):
    ...
    for period_name, period_col in [('Daily', 'date'), ('Weekly', 'week'), ('Monthly', 'month')]:
        ...
        # Aggregate instances data by game_name and time period
        instances_agg = instances_df.groupby([period_col, 'game_name']).agg({
        ...
        merged_df = visits_users_agg.merge(
            instances_agg[['time_period', 'game_name', 'instances']], 
            on=['time_period', 'game_name'], 
            how='left'
        ).fillna(0)
    ...
    print(f"SUCCESS: Created final dataset with {len(final_df)} records")
    return final_df

def main():
    """Main function to generate time series data"""
    print("GENERATING TIME SERIES DATA FOR DASHBOARD")
    print("=" * 60)
    ...
    try:
        ...
        print("\nSUCCESS: Time series data generated!")
